...
sub XMLPrint(){
    my ($module_name,$module_type,$module_description,$module_data,$module_tags,$module_critical,$module_warning)=@_;
...
if ($memusage != 0){&XMLPrint("MySQL Server MemUsage","generic_data","MySQL Server MemUsage Percent", $memusage,$tags,$DEFAULT_MEMUSAGE_CRITICAL,$DEFAULT_MEMUSAGE_WARNING);}
...
foreach (@con){
        ...
        if ($stat != 0){&XMLPrint("MySQL Server $database Queries","generic_data_inc","MySQL Server $database Queries", $value,$tags,$DEFAULT_QUERIES_CRITICAL,$DEFAULT_QUERIES_WARNING);}
